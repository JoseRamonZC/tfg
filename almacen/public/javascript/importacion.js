...
function productosjs() {
    ...
    if ($('#referencia').val().length > 2) {
        $.post(Routing.generate('products_search', {almacen: $('#almacen').val()}), {referencia: referencia, categoria: categoria, subcategoria: subcategoria, tipo: tipo}, function (respuesta) {
            if (respuesta['existe'] == 2) {
                ...
                for (i = 0; i < respuesta.length; i++) {
                    ...
                    if (respuesta[i].foto) {
                        var foto = Routing.generate('warehouse_products_photo', {almacen: $('#almacen').val(), id: respuesta[i].id});
                    } else {
...
function importacionjs(id) {
    if (id == false) {
        ...
        $.post(Routing.generate('products_add', {almacen: $('#almacen').val()}), {id: id}, function (respuesta) {
            if (respuesta['existe'] == 2) {
                ...
                if (producto) {
                    ...
                    if (respuesta['foto']) {
                        var foto = Routing.generate('warehouse_products_photo', {almacen: $('#almacen').val(), id: respuesta['id']});
                    } else {
                        var foto = "/imagenes/noimagen.png";
                    }
                    ...
                    var tabla = "<tr id='lote-" + respuesta['id'] + "'>\n\
                                    ...
                                    <td>\n\
                                        " + respuesta['nombre'] + "\
                                        <input value='" + respuesta['id'] + "' type='hidden' name='producto[]'>\n\
                                    </td>\n\
                                    <td><input type='number' name='cantidad[]' value='1' min='1' class='form-control' required></td>\n\
                                    <td><input type='text' name='lote[]' class='form-control' required></td>\n\
                                    <td><input type='text' name='fechaCaducidad[]' id='kt_flatpickr' class='form-control'></td>\n\
                                    <td><input type='number' name='precio[]' value='" + respuesta['precio'] + "' step='any' min='0' class='form-control' required></td>\n\
                                    <td class='text-center align-self-center'>\n\
                    ...
                    document.getElementById("importacion").innerHTML += tabla;
